...
def get_cluster_embeddings_full(model_name, Place_Sentences, Person_Sentences, Content_Sentences, Day_Sentences,
                                Month_Sentences, Year_Sentences, umap_flag, umap_dict):
...
def get_weighted_embeddings(cluster_embeddings_dict, complete_weights):
    time_embeddings = (complete_weights[3] * cluster_embeddings_dict['day_embeddings']) + (
            complete_weights[4] * cluster_embeddings_dict['month_embeddings']) + (
                              complete_weights[5] * cluster_embeddings_dict['year_embeddings'])
    weighted_embeddings = ((complete_weights[6] * cluster_embeddings_dict['place_embeddings']) + (
            complete_weights[7] * cluster_embeddings_dict['person_embeddings']) + (
                                   complete_weights[8] * time_embeddings) + (
                                   complete_weights[9] * cluster_embeddings_dict['content_embeddings']) / 4)
